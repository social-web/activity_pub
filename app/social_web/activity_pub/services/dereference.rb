    ...
    module Services
      ...
      class Dereference
        ...
        def call(obj)
          ...
          obj_dup.traverse_properties(depth: SocialWeb::ActivityPub[:config].max_depth) do |hash|
        ...
        def http_client
          @http_client ||= SocialWeb::ActivityPub['services.http_client'].for_actor(@actor)
        end
        ...
        def store_relationship(parent, child, prop)
          SocialWeb::ActivityPub['repositories.relationships'].store(
            parent: parent,
            child: child,
            property: prop
          )
